...
def NamedInlineRadioItems(name, id, options, **kwargs):
    return html.Div(
        id=f"div-{id}",
        style=_merge(
            {"display": "block", "margin-bottom": ".5rem", "margin-top": ".5rem"},
            kwargs.get("style", {}),
        ),
        children=[
            f"{name}:",
            dcc.RadioItems(
                ...
                style={"display": "inline-block", "margin-left": "1rem", "padding-left":"1rem"},
            ),
        ],
        **_omit(["style"], kwargs),
    )
